...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        self.button_don = wx.Button(self.panel, -1, l_donate[1])
        ...
        #UPDATE
        self.box0 = TransparentBox(self.panel, -1 ,l_updater[1])
        ...
        #LAUNCH
        self.box2 = TransparentBox(self.panel, -1, l_launch[1])
        ...
        #PAYPAL
        self.box3 = TransparentBox(self.panel, -1, l_paypal[1])
        ...
        #CONFIG
        self.box4 = TransparentBox(self.panel, -1, l_settings[1])
        ...
        #PROGRESS BAR
        self.box5= TransparentBox(self.panel, -1, l_progress[1])
        ...
        #SERV ON/OFF
        self.box6= TransparentBox(self.panel, -1, l_onoff[1])
        ...
        #NEWS
        self.box7= TransparentBox(self.panel, -1, l_news[1])
        ...
        with open(u"fix.conf",u"r")as fix_r:
            ...
            if mem_fix==u"0":
                try:
                    ...
                    winreg.SetValueEx(key, u"Launcher", 0,  winreg.REG_SZ, u"Updater_Launcher_ENG.exe")
                    ...
                    self.AffichTxt.SetForegroundColour(u"FOREST GREEN")
                    self.AffichTxt.AppendText(l_g_patch[1])
                except:
                    self.AffichTxt.Clear()
                    self.AffichTxt.SetFont(wx.Font(12, wx.DEFAULT, wx.NORMAL, wx.BOLD))
                    self.AffichTxt.SetForegroundColour(u"RED")
                    self.AffichTxt.AppendText(l_err_patch[1]+"\n\n"+l_admin[1])     #l_admin language key (del if needed)
                    self.chck_maj.Disable()
            else:
                self.AffichTxt.Clear()
                self.AffichTxt.SetFont(wx.Font(12, wx.DEFAULT, wx.NORMAL, wx.BOLD))
                self.AffichTxt.SetForegroundColour(u"RED")
                self.AffichTxt.AppendText(l_patch[1])
...
###########################################################################################
    ...
    def no_internet(self):
        global no_net
        alert=wx.MessageDialog(self,l_err_net[1], l_net_title[1],wx.OK | wx.ICON_ERROR)
        alert.ShowModal()
        self.start_g.Disable()
        no_net=1
    ...
    def Recup_Params(self):# Recup donnees de client_config.conf
        ...
        try:
            ...
            self.AffichTxt.Clear()
            self.AffichTxt.SetForegroundColour(u"RED")
            self.AffichTxt.AppendText(l_err_net[1])
    ...
    def CreerBarreOutils(self): # Pour creation de la barre d'outils
        ...
        tool_settings=toolbar.AddTool(ID_SETTINGS,u"Settings",img_settings,shortHelp=str(l_aide_set[1]))

        toolbar.AddSeparator()

        tool_exit=toolbar.AddTool(ID_EXIT,u"Exit",img_exit,shortHelp=str(l_aide_exit[1]))
    ...
    def settings(self,evt):
        ...
        dlg=GetData(self,title=l_set_dlg[1])
    ...
    def launch_g(self,evt):
        ...
        if file_is_ok==1:
            ...
            alert=wx.MessageDialog(self,l_bad_exe[1], l_exe[1] ,wx.OK | wx.ICON_ERROR)
            alert.ShowModal()
            self.start_g.Disable()
            self.AffichTxt.Clear()
            self.AffichTxt.SetForegroundColour(u"RED")
            self.AffichTxt.AppendText(l_err_bat[1])
                ...
                if folder_ok==1 or folder_ok==2:
                    ...
                    try:
                        ...
                        if len(links_pak)!=0:
                            ...
                            self.AffichTxt.Clear()
                            self.AffichTxt.SetForegroundColour(u"RED")
                            self.AffichTxt.AppendText(l_err_parse[1])
                            return
                        no_net=0
                    except:
                        self.no_internet()
                    ...
                    if lst_ver_same!=[]:
                        ...
                        while j<len(lst_ver_same):
                            ...
                            if checksum_fichier_local!=checksum_fichier_dist:
                                ...
                                if tab_line[4]!=u"none":
                                    self.txtProg.SetLabel(l_txtProg[1]+nom_de_pak_dist)
                        ...
                        if lst_ver_diff!=[]:
                            ...
                            while x<len(lst_ver_diff):
                                ...
                                self.gauge.SetRange(n)
                                if tab_line[4]!=u"none":
                                    self.txtProg.SetLabel(l_txtProg[1]+nom_de_pak)
                            ...
                            self.AffichTxt.Clear()
                            self.AffichTxt.SetForegroundColour(u"RED")
                            self.AffichTxt.AppendText(l_err_ext[1])
                            for elem in liste_loc_ext:
                                self.AffichTxt.AppendText(u"-"+str(elem)+u"\n")
                            self.AffichTxt.AppendText(l_ici[1])
                            self.start_g.Disable()
                else:
                    alert=wx.MessageDialog(self,l_bad_file[1], l_404_file[1] ,wx.OK | wx.ICON_ERROR)
                    alert.ShowModal()
                    self.start_g.Disable()
        else:
            alert=wx.MessageDialog(self,l_bad_serv[1], l_404[1] ,wx.OK | wx.ICON_ERROR)
            alert.ShowModal()
            self.start_g.Disable()
...
class GetData(wx.Dialog):
    def __init__(self, parent,title):
        ...
        #file_config
        self.lbl_file_config = wx.StaticText(self.panel, label=l_file_config[1], pos=(20,20))
        self.file_config = wx.TextCtrl(self.panel, value=u"http://", pos=(180,20), size=(300,-1))
        self.file_config_txt = wx.StaticText(self.panel,label=l_file_config_txt[1],pos=(180,50))
        ...
        self.secu_txt = wx.StaticText(self.panel,label=l_secu_txt[1],pos=(10,100))
        self.secu_txt.SetFont(wx.Font(10, wx.DEFAULT , wx.NORMAL, wx.BOLD))
        self.secu_txt.SetForegroundColour(u"RED")
        self.secu_box=wx.CheckBox(self.panel, label = l_check[1],pos = (290,160))
